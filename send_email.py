...
def compose_email(template, name, data_dict):
  ...
  composed = template.substitute(
    ...
    GITHUB_LINK='https://github.com/Astrowests/capstone-emailAutomation'
  )
  return composed
...
def create_plot(file_path="data_input/data.csv", id=['936', '1178']):
  ...
  grouped = campaigns.groupby(
        by=['campaign_id', 'age', 'reporting_start'], as_index=False)['campaign_id','age','reporting_start','total_conversion'].sum()
